...
Fixed App Router routes are supported whether they are statically prerendered or rendered per
request. Parameterized SSR routes such as `/posts/[slug]` require a future explicit-route mechanism;
concrete routes generated by `generateStaticParams()` are supported.
